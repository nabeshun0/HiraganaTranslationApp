    ...
    private func fetchHiragana() {
        ...
        Session.send(HiraganaTranslatorAPI.Request(sentence: sentence), callbackQueue: nil) { [weak self] (result) in
            ...
            case .failure(.responseError(let error as APPErrorCode)):
                let errorMessage = ErrorMessage(title: R.string.localizable.error(), message: error.localizedDescription)
                self.delegate?.hiraganaTranslatorModel(self, didRecieve: errorMessage)
            case .failure(let error):
                let errorMessage = ErrorMessage(title: R.string.localizable.unknownError(), message: error.localizedDescription)
                self.delegate?.hiraganaTranslatorModel(self, didRecieve: errorMessage)
            }
        }
    }
